...
#let incl(src, label, lang: "typst", sourceline: true) = {
  ...
  code(lang: lang, (if sourceline {fstline + "\n"} else { }) + lines.slice(start + 1, end).join("\n"))
}

#let full(src, lang: "typst", sourceline: true) = {
  let lines = read("/" + src)
  // TODO: allow disabling the header.
