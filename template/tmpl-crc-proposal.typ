...
#let crc-project(
  ...
  metadata = if metadata != none { metadata.value } else { (:) }
  let colors = if "colors" in metadata { metadata.colors } else { (:) }
  let projects = if "project" in metadata { metadata.project } else { (:) }
  let aux = if "aux" in metadata { metadata.aux } else { (:) }
  let person = if "person" in metadata { metadata.person } else { (:) }
  ...
  set page(
    background: context {
      let c = if colors.research.keys().contains(lower(projects.at(lower(number)).number.at(0))) { 
          colors.research.at(lower(projects.at(lower(number)).number.at(0)))
      } else if colors.mgk.keys().contains(lower(projects.at(lower(number)).number)) { 
          colors.mgk.at(lower(projects.at(lower(number)).number))
      } else if colors.inf.keys().contains(lower(projects.at(lower(number)).number)) { 
          colors.inf.at(lower(projects.at(lower(number)).number)) 
      } else if colors.wiko.keys().contains(lower(projects.at(lower(number)).number)) { 
          colors.wiko.at(lower(projects.at(lower(number)).number)) 
      } else if colors.transfer.keys().contains(lower(projects.at(lower(number)).number.at(0))) { 
          colors.transfer.at(lower(projects.at(lower(number)).number.at(0))) 
      } else if colors.admin.keys().contains(lower(projects.at(lower(number)).number)) { 
          colors.admin.at(lower(projects.at(lower(number)).number)) 
      } else { colors.default }
      ...
      align(
        algn, 
        grid(
          rows: (1fr)*projects.keys().len(),
        ..aux.project.map(it => { 
            if it == projects.at(lower(number)).number { 
    ...
    header: context { align(center, [#projects.at(lower(number)).pi.map(it => person.at(it).name).join([ #sym.dot.c ]) | #projects.at(lower(number)).name]) }
  )
    ...
    === Project title: #projects.at(lower(number)).name
    
    #if projects.at(lower(number)).type in ("research", "transfer") and projects.at(lower(number)).status in ("C", "N") [
      === Research areas: #projects.at(lower(number)).area.join([, ])
    ]
    
    === Project leaders
    
    // the boxes containing personal information about the subproject's PIs
    #let pi = projects.at(lower(number)).pi.map(it => personInfo(it))
    #grid(
      columns: (1fr, 1fr),
      column-gutter: 10pt,
      row-gutter: 15pt,
    ...
    #let questions = ()
    #if projects.at(lower(number)).pi.len() > 1 {
      questions = aux.questions.person.multiple
    } else {
      questions = aux.questions.person.single
    } 
    
    - #questions.Q1 #projects.at(lower(number)).pi.map(p => [*#ifYesNo(person.at(p).Q1)* (#person.at(p).name)]).join([, ])
    - #questions.Q2 #projects.at(lower(number)).pi.map(p => [*#ifYesNo(person.at(p).Q2)* (#person.at(p).name)]).join([, ])
    #if projects.at(lower(number)).pi.map(p => person.at(p).Q2).any(it => it) { 
      [ - #questions.Q3: #projects.at(lower(number)).pi.filter(p => person.at(p).Q2).map(p => if person.at(p).Q2 { 
        [*#person.at(p).Q3* (#person.at(p).name)]
      }).join([, ])]}
    #if projects.at(lower(number)).pi.map(p => person.at(p).Q2).any(it => it) { 
      [ - #questions.Q4: #projects.at(lower(number)).pi.filter(p => person.at(p).Q2).map(p => if person.at(p).Q2 { 
        [*#person.at(p).Q4* (#person.at(p).name)]
      }).join([, ])]}   
    #if projects.at(lower(number)).pi.map(p => person.at(p).Q5).any(it => it) { 
      [ - #questions.Q5 #projects.at(lower(number)).pi.map(p => [*#ifYesNo(person.at(p).Q5)* (#person.at(p).name)]).join([, ])]
    }
    - #questions.Q6: #projects.at(lower(number)).pi.map(p => [*#ifYesNo(person.at(p).Q6)* (#person.at(p).name)]).join([, ])
    
    
    #if projects.at(lower(number)).type == "transfer" [
      === Application partner
    ...
    #if projects.at(lower(number)).type in ("research", "infrastructure", "wiko", "transfer") and projects.at(lower(number)).status in ("C", "N") [
      #block(
        breakable: false,
        [
          === Legal issues
          ...
          #grid(
            ...
            gutter: 7pt,
            [1.], aux.questions.legal.Q1, [#ifYesNoIcon(projects.at(lower(number)).Q1)],
            ..if projects.at(lower(number)).Q1 { 
              ([], aux.questions.legal.Q1a, [#ifYesNoIcon(projects.at(lower(number)).Q1a)])
            },
            [2.], aux.questions.legal.Q2, [#ifYesNoIcon(projects.at(lower(number)).Q2)],
            ..if projects.at(lower(number)).Q2 { 
              ([], aux.questions.legal.Q2a, [#ifYesNoIcon(projects.at(lower(number)).Q2a)])
            },
            [3.], aux.questions.legal.Q3, [#ifYesNoIcon(projects.at(lower(number)).Q3)],
            [4.], aux.questions.legal.Q4, [#ifYesNoIcon(projects.at(lower(number)).Q4)],
            [5.], aux.questions.legal.Q5, [#ifYesNoIcon(projects.at(lower(number)).Q5)],
            ..if projects.at(lower(number)).Q5 { 
              ([], aux.questions.legal.Q5a, [#ifYesNoIcon(projects.at(lower(number)).Q5a)])
            },
            [6.], aux.questions.legal.Q6, [#ifYesNoIcon(projects.at(lower(number)).Q6)],
            [7.], aux.questions.legal.Q7, [#ifYesNoIcon(projects.at(lower(number)).Q7)],
            ..if projects.at(lower(number)).Q7 { 
              ([], projects.at(lower(number)).get().aux.questions.legal.Q7a, [#ifYesNoIcon(projects.at(lower(number)).Q7a)])
            }
          )
  ...
  if bib != none {
    ...
    let highlight = if s-project-refs.get().keys().contains(prefix) { s-project-refs.get().at(prefix) } else { () }
    ...
    context if body.has("children") {
      ...
      let bib_ = [
        ...
        #show regex(projects.at(lower(number)).pi.map(pi => person.at(pi).name + ", " + person.at(pi).name-first.at(0) + ".").join("|")): set text(weight: "bold")
      ]
  
      // check which type of bibliography is needed, 
      if projects.at(lower(number)).type == "inf" or projects.at(lower(number)).type in ("research", "transfer") and projects.at(lower(number)).status == "E" {
        bib_ = bib_ + [
